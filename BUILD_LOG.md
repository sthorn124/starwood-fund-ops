...
**Promotion checkpoint: current through 2026-09-22 — Fix session: intake confirmation state, draw-number collision handling — level with the log tail.** A session touching promotion refuses to call itself complete if this checkpoint lags the log tail by more than one session.
