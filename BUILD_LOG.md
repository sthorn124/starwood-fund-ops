...
**Promotion checkpoint: current through 2026-09-22 — Fix session: current approver action on the draw summary — level with the log tail.** A session touching promotion refuses to call itself complete if this checkpoint lags the log tail by more than one session.
